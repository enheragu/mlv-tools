console.log('[comparison-app-core.js] Script loaded and running');
(function () {
  ...
  function init(options) {
    ...
    function drawDecisionLinksIn(rootElement) {
      ...
      flows.forEach(function (flow) {
        ...
        function addPath(x1, y1, x2, y2, cls, xMiddleOverride) {
          // Redondeo clásico: horizontal, arco, vertical, arco, horizontal
          var r = 8;
          var dx = Math.abs(x2 - x1);
          var dy = Math.abs(y2 - y1);
          if (Math.abs(y2 - y1) < 1e-2) {
            return '<path class="' + cls + '" d="M ' + x1.toFixed(2) + ' ' + y1.toFixed(2) + ' H ' + x2.toFixed(2) + '" />';
          }
          if (dx < r * 2 || dy < r * 2) {
            return '<path class="' + cls + '" d="M ' + x1.toFixed(2) + ' ' + y1.toFixed(2) + ' L ' + x2.toFixed(2) + ' ' + y2.toFixed(2) + '" />';
          }
          var xMiddle = (typeof xMiddleOverride === 'number') ? xMiddleOverride : (x1 + x2) / 2;
          var yDir = y2 > y1 ? 1 : -1;
          return [
            '<path class="' + cls + '" d="M ' + x1.toFixed(2) + ' ' + y1.toFixed(2),
            'H ' + (xMiddle - r).toFixed(2),
            'A ' + r + ' ' + r + ' 0 0 ' + (yDir > 0 ? 1 : 0) + ' ' + xMiddle.toFixed(2) + ' ' + (y1 + r * yDir).toFixed(2),
            'V ' + (y2 - r * yDir).toFixed(2),
            'A ' + r + ' ' + r + ' 0 0 ' + (yDir > 0 ? 0 : 1) + ' ' + (xMiddle + r).toFixed(2) + ' ' + y2.toFixed(2),
            'H ' + x2.toFixed(2),
            '" />'
          ].join(' ');
        }
        ...
        if (activeStart && byLevel[0] && byLevel[0].length) {
          ...
          byLevel[0].forEach(function (btn) {
            ...
            var pathStr = addPath(s0.x, s0.y, t0.x, t0.y, 'decision-link decision-link--l1' + (isActive0 ? ' is-active' : '') + (isBest0 ? ' decision-link--best' : ''));
            if (isBest0) bestPaths.push(pathStr); else paths.push(pathStr);
          });
        }
        ...
        levels.forEach(function (level) {
          ...
          next.forEach(function (child) {
            ...
            var pathStr = addPath(s.x, s.y, t.x, t.y, 'decision-link decision-link--l2' + (isActive ? ' is-active' : '') + (isBest ? ' decision-link--best' : ''));
            if (isBest) bestPaths.push(pathStr); else paths.push(pathStr);
          });
        });

        svg.innerHTML = paths.concat(bestPaths).join('');
      });
    }
    ...
    function buildDataFromCsvTable(raw, tableCfg) {
      ...
      var factors = Array.isArray(tableCfg && tableCfg.factors) ? tableCfg.factors : [];
      var seedCol = tableCfg && typeof tableCfg.seedColumn === 'string' ? tableCfg.seedColumn.trim() : '';
      var dataByModel = {};
      var order = [];
      var validRows = [];
      var modelFactors = {};
      var displayByModel = {};
      var seedDataByModel = {}; // model → [{ seed, value }]
      ...
      rows.forEach(function (row) {
        ...
        if (!Object.prototype.hasOwnProperty.call(dataByModel, key)) {
          ...
          if (seedCol) seedDataByModel[key] = [];
        }
        dataByModel[key].push(metricValue);

        // Collect seed→metric mapping if seedColumn configured
        if (seedCol) {
          var seedVal = getRowField(row, seedCol, headerMap);
          if (seedVal != null && String(seedVal).trim()) {
            seedDataByModel[key].push({ seed: String(seedVal).trim(), value: metricValue });
          }
        }
      });
      ...
      if (seedCol && Object.keys(seedDataByModel).length) {
        result.seedDataByModel = seedDataByModel;
      }
      return result;
    }
    ...
    function renderModelSummaryTable() {
      ...
      if (!tbody) return;
      var ordered = getSelectedOrdered();
      if (!ordered.length) {
        tbody.innerHTML = '';
        return;
      }
    ...
    function renderChart() {
      ...
      if (typeof options.onRenderChart === 'function') {
        options.onRenderChart({
          selected: getSelectedOrdered(),
          dataByModel: state.dataByModel,
          displayByModel: state.modelDisplayByKey,
          colorByModel: state.modelColorByKey,
          metric: state.metric,
          preset: state.preset,
          conditionMeta: state.conditionMeta,
          seedDataByModel: state.seedDataByModel,
        });
      }
    }
    ...
    async function runSimulation() {
      ...
      if (typeof options.onSimulationStart === 'function') {
      ...
      if (metricInfo.table && metricInfo.table.file) {
        fetch(metricInfo.table.file, { cache: 'no-store' })
          ...
          .then(function (raw) {
            ...
            state.seedDataByModel = parsed.seedDataByModel || null;
